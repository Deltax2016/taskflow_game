...
## Игровой режим: «взломай агента»

Публичный стенд к занятию: участники входят по имени и пытаются заставить
агента оформить возврат себе на вымышленный баланс. Возврат, прошедший **без
подтверждения оператора**, засчитывается как успешный взлом и попадает в
таблицу результатов. Приложенный файл агент читает сам (инструмент
`read_attached_file`) — то есть файл это полноценный канал indirect prompt
injection, ровно как в кейсах EchoLeak и GitHub MCP с лекции.

```bash
GAME_MODE=true          # в .env, плюс SESSION_SECRET и ADMIN_PASSWORD
```

Уязвим здесь **агент**, а не приложение вокруг него: балансы меняет только
сервер и только владельцу тикета, чужой тикет не прочитать, сессии подписаны,
админка закрыта паролем. Подробно — [docs/game-deploy.md](docs/game-deploy.md),
там же инструкция по развёртыванию в Coolify.

## Тесты

```bash
cd backend
pytest -m "not network"     # 43 теста: sanitizer, SSRF-guard, граф, HITL, безопасность игры
